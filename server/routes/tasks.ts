...
export function registerTaskRoutes(app: any) {
  ...
  router.patch("/:id", requireAuth, async (req: Request, res: Response) => {
    ...
    if (requester.role === "DEVELOPER") {
      ...
      if (task.assigneeId !== requester.id) return res.status(403).json({ error: "forbidden" });
      const allowed: Partial<Task> = {};
      if (parsed.data.status) allowed.status = parsed.data.status as Task["status"];
      if (parsed.data.description) {
        // push as a comment
        const comment = { id: randomUUID(), text: parsed.data.description, userId: requester.id, createdAt: new Date().toISOString() };
  ...
  router.post("/:id/comments", requireAuth, (req: Request, res: Response) => {
    ...
    const task = db.tasks.find((t) => t.id === req.params.id);
    if (!task) return res.status(404).json({ error: "Task not found" });
    const comment = { id: randomUUID(), text, userId: requester.id, createdAt: new Date().toISOString() };
    task.comments.push(comment);
    task.updatedAt = new Date().toISOString();
