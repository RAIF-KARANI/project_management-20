...
export async function initDb() {
  ...
  if (cnt === 0) {
    ...
    const insertText =
      "INSERT INTO users(id,name,email,role,password,created_at) VALUES($1,$2,$3,$4,$5,$6)";
...
export async function verifyUserCredentials(
  ...
  const { hashPassword } = await import("../utils/password");
  const res = await pool.query(
    "SELECT id, name, email, role, password, created_at FROM users WHERE lower(email)=lower($1) LIMIT 1",
    [email],
  );
  if (!res.rows[0]) return null;
  const r = res.rows[0];
...
export async function createUser(
  ...
  if ((user as any).password) {
    ...
    await pool.query(
      "INSERT INTO users(id,name,email,role,password,created_at) VALUES($1,$2,$3,$4,$5,$6)",
      [id, user.name, user.email, user.role, h, now],
    );
  } else {
    await pool.query(
      "INSERT INTO users(id,name,email,role,created_at) VALUES($1,$2,$3,$4,$5)",
      [id, user.name, user.email, user.role, now],
    );
  }
  return {
    id,
